...
from parsingrcnn.utils.collections import AttrDict
...
# Evaluate detections with the VOC dataset eval code even if it's not the
# evaluation code for the dataset
__C.TEST.FORCE_VOC_DATASET_EVAL = False
...
# See configs/test_time_aug/e2e_parsing_rcnn_R-50-FPN_2x.yaml for an example
# ---------------------------------------------------------------------------- #
...
# Horizontal flip at the original scale (id transform)
__C.TEST.PARSING_AUG.H_FLIP = False

# Each scale is the pixel size of an image's shortest side
__C.TEST.PARSING_AUG.SCALES = ()

# Max pixel size of the longer side
__C.TEST.PARSING_AUG.MAX_SIZE = 4000

# Horizontal flip at each scale
__C.TEST.PARSING_AUG.SCALE_H_FLIP = False

# Apply scaling based on object size
__C.TEST.PARSING_AUG.SCALE_SIZE_DEP = False
__C.TEST.PARSING_AUG.AREA_TH = 180**2

# Each aspect ratio is relative to image width
__C.TEST.PARSING_AUG.ASPECT_RATIOS = ()

# Horizontal flip at each aspect ratio
__C.TEST.PARSING_AUG.ASPECT_RATIO_H_FLIP = False
...
# Indicates the model makes UV predictions (as in DensePose R-CNN)
...
# Whether to load pytorch pretrained weights during training
# If True, __C.TRAIN.WEIGHTS must be .pth file
__C.MODEL.PYTORCH_WEIGHTS_MAPPING = False
...
# Parameters for roi_mask_align:
# Value to shift roi under feature map scale
__C.FAST_RCNN.ROI_XFORM_SPATIAL_SHIFT = 0.

# Pattern to keep half of roi:
# 0 refer to keep original roi
# 1 refer to keep lift half of roi
# 2 refer to keep right half of roi
# 3 refer to keep upper half of roi
# 4 refer to keep bottom half of roi
__C.FAST_RCNN.ROI_XFORM_HALF_PART = 0

# Value to rescale an roi's width and height under feature map scale
__C.FAST_RCNN.ROI_XFORM_ROI_SCALE = 1.

# Light Head
# Kernel size of 1xN conv
__C.FAST_RCNN.LIGHT_HEAD_KERNEL = 15

# Middle dim of conv, 256 for coco, 128 for smalle scale dataset
__C.FAST_RCNN.LIGHT_HEAD_MIDDLE_DIM = 256

# Output dim of conv, 490 for coco, 256 for smalle scale dataset
__C.FAST_RCNN.LIGHT_HEAD_OUTPUT_DIM = 490

# FC dim before cls_core, (2048, 2048) for coco, (256, 1024) for smalle scale dataset
__C.FAST_RCNN.LIGHT_HEAD_FC_DIM = (2048, 2048)
...
# Stride of the backbone, 64 for common model, 16 for detnet
__C.FPN.BACKBONE_STRIDE = 64
...
# Generate kps using less time
__C.KRCNN.FAST_KPS = False
...
# keypoint roi max batch size 
__C.KRCNN.ROI_BATCH_SIZE = -1
...
# Dimension for ASPP head
__C.KRCNN.ASPP_DIM = 256

# Dilation for aspp head
__C.KRCNN.ASPP_DILATION = (6, 12, 18)

# Number of stacked Conv layers in aspp head before aspp
__C.KRCNN.NUM_CONVS_BEFORE_ASPP = 0

# Number of stacked Conv layers in aspp head after aspp
__C.KRCNN.NUM_CONVS_AFTER_ASPP = 0
...
__C.KRCNN.GAUSS_HEATMAP = False

__C.KRCNN.GAUSS_HEATMAP_TEST = False

# Use Finest level ROI for keypoints head
__C.KRCNN.FINEST_LEVEL_ROI = False


# ---------------------------------------------------------------------------- #
# Mask R-CNN options ("BRCNN" means parsing R-CNN)
# ---------------------------------------------------------------------------- #
__C.PRCNN = AttrDict()

# The type of RoI head to use for instance parsing prediction
# The string must match a function this is imported in modeling.model_builder
# (e.g., 'mask_rcnn_heads.ResNet_mask_rcnn_fcn_head_v1up4convs')
__C.PRCNN.ROI_PARSING_HEAD = ''

# Number of parsings in the dataset
__C.PRCNN.NUM_PARSING = -1

__C.PRCNN.LEFT_RIGHT = ()

__C.PRCNN.ORIGINAL_OUTPUT = True

# Resolution of parsing predictions
__C.PRCNN.RESOLUTION = 14

# Use bilinear interpolation to upsample the final heatmap by this factor
__C.PRCNN.UP_SCALE = -1

# Apply a ConvTranspose layer to the hidden representation computed by the
# parsing head prior to predicting the per-parsing heatmaps
__C.PRCNN.USE_DECONV = False
# Channel dimension of the hidden representation produced by the ConvTranspose
__C.PRCNN.DECONV_DIM = 256

# Use a ConvTranspose layer to predict the per-keypoint heatmaps
__C.PRCNN.USE_DECONV_OUTPUT = False

# Size of the kernels to use in all ConvTranspose operations
__C.PRCNN.DECONV_KERNEL = 4

# Dimension of the hidden representation output by the parsing head
__C.PRCNN.CONV_HEAD_DIM = 256

# Conv kernel size used in the keypoint head
__C.PRCNN.CONV_HEAD_KERNEL = 3

# Number of stacked Conv layers in keypoint head
__C.PRCNN.NUM_STACKED_CONVS = 8

# Dimension for ASPP head
__C.PRCNN.ASPP_DIM = 256

# Dilation for aspp head
__C.PRCNN.ASPP_DILATION = (6, 12, 18)

# Number of stacked Conv layers in aspp head before aspp
__C.PRCNN.NUM_CONVS_BEFORE_ASPP = 0

# Number of stacked Conv layers in aspp head after aspp
__C.PRCNN.NUM_CONVS_AFTER_ASPP = 0

# Dimension for psp head
__C.PRCNN.PSP_DIM = 256

# Pool scales for psp head
__C.PRCNN.PSP_POOL_SCALES = (1, 2, 4, 7) 

# Number of stacked Conv layers in psp head before psp
__C.PRCNN.NUM_CONVS_BEFORE_PSP = 0

# Number of stacked Conv layers in psp head after psp
__C.PRCNN.NUM_CONVS_AFTER_PSP = 0

# Weight initialization method for the parsing head and mask output layers
__C.PRCNN.CONV_INIT = 'GaussianFill'

# parsing roi max batch size 
__C.PRCNN.ROI_BATCH_SIZE = -1

# RoI transformation function and associated options
__C.PRCNN.ROI_XFORM_METHOD = 'RoIAlign'

# RoI transformation function (e.g., RoIPool or RoIAlign)
__C.PRCNN.ROI_XFORM_RESOLUTION = 7

# Number of grid sampling points in RoIAlign (usually use 2)
# Only applies to RoIAlign
__C.PRCNN.ROI_XFORM_SAMPLING_RATIO = 0

# Multi-task loss weight for parsings
__C.PRCNN.WEIGHT_LOSS_PARSING = 1.0

# Generating segmentation results
__C.PRCNN.SCORE = 0.3

# Evaluate the AP metrics
__C.PRCNN.EVAL_AP = False

# Use Finest level ROI for parsing head
...
# The type of RoI head to use for UV prediction
__C.UVRCNN.ROI_HEAD = ''

# Output size (and size loss is computed on), e.g., 56x56
__C.UVRCNN.HEATMAP_SIZE = -1

# Use bilinear interpolation to upsample the final heatmap by this factor
__C.UVRCNN.UP_SCALE = -1

# Apply a ConvTranspose layer to the features prior to predicting the heatmaps
__C.UVRCNN.USE_DECONV = False
# Channel dimension of the hidden representation produced by the ConvTranspose
__C.UVRCNN.DECONV_DIM = 256
# Use a ConvTranspose layer to predict the heatmaps
__C.UVRCNN.USE_DECONV_OUTPUT = False
# Use dilation in the UV head
__C.UVRCNN.DILATION = 1
# Size of the kernels to use in all ConvTranspose operations
__C.UVRCNN.DECONV_KERNEL = 4

# Number of patches in the dataset
__C.UVRCNN.NUM_PATCHES = -1

# Number of stacked Conv layers in UV head
__C.UVRCNN.NUM_STACKED_CONVS = 8

# Dimension for ASPP head
__C.UVRCNN.ASPP_DIM = 256

# Dilation for aspp head
__C.UVRCNN.ASPP_DILATION = (6, 12, 18)

# Number of stacked Conv layers in aspp head before aspp
__C.UVRCNN.NUM_CONVS_BEFORE_ASPP = 0

# Number of stacked Conv layers in aspp head after aspp
__C.UVRCNN.NUM_CONVS_AFTER_ASPP = 0

# Dimension of the hidden representation output by the UV head
__C.UVRCNN.CONV_HEAD_DIM = 256
# Conv kernel size used in the UV head
__C.UVRCNN.CONV_HEAD_KERNEL = 3
# Conv kernel weight filling function
__C.UVRCNN.CONV_INIT = 'GaussianFill'

# Standard ROI XFORM options (see FAST_RCNN or MRCNN options)
__C.UVRCNN.ROI_XFORM_METHOD = 'RoIAlign'
__C.UVRCNN.ROI_XFORM_RESOLUTION = 7
__C.UVRCNN.ROI_XFORM_SAMPLING_RATIO = 0

# Weights
__C.UVRCNN.INDEX_WEIGHTS = 5.0
__C.UVRCNN.PART_WEIGHTS = 1.0
__C.UVRCNN.POINT_REGRESSION_WEIGHTS = 0.001

# Train only with images that have uv annotations
__C.UVRCNN.UV_IMS = False

# Evaluate the GPSM metric
__C.UVRCNN.GPSM_ON = False

# Use Finest level ROI for uv head
...
# Use a non-local block right before the last residual block of stage "res4"
__C.RESNETS.USE_NONLOCAL = False

# Use a deformable convolution layers replace the normal convolution layers
__C.RESNETS.USE_DEFORM = False
...
# ---------------------------------------------------------------------------- #
# Airs options ("Airs" = Air, AirX, ResNet_priv and ResNeXt_priv)
# ---------------------------------------------------------------------------- #
__C.AIRS = AttrDict()

# Number of groups to use; 1 ==> Air/ResNet; > 1 ==> AirX/ResNeXt
__C.AIRS.NUM_GROUPS = 1

# Baseline width of each group
__C.AIRS.WIDTH_PER_GROUP = 64

# Baseline width of outplane
__C.AIRS.WIDTH_OUTPLANE = 256

# Residual transformation function
__C.AIRS.TRANS_FUNC = 'bottleneck_transformation'

# AirNet's stem function (conv1, conv2, conv3 and pool1)
__C.AIRS.STEM_FUNC = 'basic_bn_stem'

# AirNet's shortcut function
__C.AIRS.SHORTCUT_FUNC = 'basic_bn_shortcut'

# Apply dilation in stage "air5/res5"
__C.AIRS.C5_DILATION = 1

# Use a non-local block right before the last residual block of stage "air4"
__C.AIRS.USE_NONLOCAL = False

# Use a deformable convolution layers replace the normal convolution layers
__C.AIRS.USE_DEFORM = False

# Freeze model weights before and including which block.
# Choices: [0, 2, 3, 4, 5]. O means not fixed. First conv and bn are defaults to
# be fixed.
__C.AIRS.FREEZE_AT = 2

__C.AIRS.BLOCK_COUNTS = (3, 4, 6, 3)

# Use GroupNorm instead of BatchNorm
__C.AIRS.USE_GN = False

# ---------------------------------------------------------------------------- #
# Attirs options ("Attirs" = Attir, AttirX)
# ---------------------------------------------------------------------------- #
__C.ATTIRS = AttrDict()

# Number of groups to use; 1 ==> Attir; > 1 ==> AttirX
__C.ATTIRS.NUM_GROUPS = 1

# Baseline width of each group
__C.ATTIRS.WIDTH_PER_GROUP = 64

# Baseline width of outplane
__C.ATTIRS.WIDTH_OUTPLANE = 256

# Residual transformation function
__C.ATTIRS.TRANS_FUNC = 'bottleneck_transformation'

# AttirNet's stem function (conv1, conv2, conv3 and pool1)
__C.ATTIRS.STEM_FUNC = 'basic_bn_stem'

# AttirNet's shortcut function
__C.ATTIRS.SHORTCUT_FUNC = 'basic_bn_shortcut'

# Apply dilation in stage "attir5"
__C.ATTIRS.C5_DILATION = 1

# Freeze model weights before and including which block.
# Choices: [0, 2, 3, 4, 5]. O means not fixed. First conv and bn are defaults to
# be fixed.
__C.ATTIRS.FREEZE_AT = 2

__C.ATTIRS.BLOCK_COUNTS = (3, 4, 6, 3)

__C.ATTIRS.COMPRESSION_RATIO = 2

# Use GroupNorm instead of BatchNorm
__C.ATTIRS.USE_GN = False

# ---------------------------------------------------------------------------- #
# DetNets options
# ---------------------------------------------------------------------------- #
__C.DETNETS = AttrDict()

# Number of groups to use; 1 ==> DetNet; > 1 ==> DetNeXt
__C.DETNETS.NUM_GROUPS = 1

# Baseline width of each group
__C.DETNETS.WIDTH_PER_GROUP = 64

# Baseline width of outplane
__C.DETNETS.WIDTH_OUTPLANE = 256

# Residual transformation function
__C.DETNETS.TRANS_FUNC = 'bottleneck_transformation'

# AirNet's stem function (conv1, conv2, conv3 and pool1)
__C.DETNETS.STEM_FUNC = 'basic_bn_stem'

# DETNETS's shortcut function
__C.DETNETS.SHORTCUT_FUNC = 'basic_bn_shortcut'

# Freeze model weights before and including which block.
# Choices: [0, 2, 3, 4, 5]. O means not fixed. First conv and bn are defaults to
# be fixed.
__C.DETNETS.FREEZE_AT = 2

__C.DETNETS.BLOCK_COUNTS = (3, 4, 6, 3, 3)

# Use GroupNorm instead of BatchNorm
__C.DETNETS.USE_GN = False
__C.DETNETS.USE_SN = False

__C.DETNETS.SN = AttrDict()
__C.DETNETS.SN.USE_BN = True

# ---------------------------------------------------------------------------- #
# DetAirs options
# ---------------------------------------------------------------------------- #
__C.DETAIRS = AttrDict()

# Number of groups to use; 1 ==> DetAir; > 1 ==> DetAirX
__C.DETAIRS.NUM_GROUPS = 1

# Baseline width of each group
__C.DETAIRS.WIDTH_PER_GROUP = 64

# Baseline width of outplane
__C.DETAIRS.WIDTH_OUTPLANE = 256

# Residual transformation function
__C.DETAIRS.TRANS_FUNC = 'bottleneck_transformation'

# AirNet's stem function (conv1, conv2, conv3 and pool1)
__C.DETAIRS.STEM_FUNC = 'basic_bn_stem'

# DETAIRS's shortcut function
__C.DETAIRS.SHORTCUT_FUNC = 'basic_bn_shortcut'

# Freeze model weights before and including which block.
# Choices: [0, 2, 3, 4, 5, 6]. O means not fixed. First conv and bn are defaults to
# be fixed.
__C.DETAIRS.FREEZE_AT = 2

__C.DETAIRS.BLOCK_COUNTS = (3, 4, 6, 3, 3)

# Use GroupNorm instead of BatchNorm
__C.DETAIRS.USE_GN = False
__C.DETAIRS.USE_SN = False

__C.DETAIRS.SN = AttrDict()
__C.DETAIRS.SN.USE_BN = True

# ---------------------------------------------------------------------------- #
# CROSSNET options
# ---------------------------------------------------------------------------- #
__C.CROSSNET = AttrDict()

__C.CROSSNET.BASE_WIDTH = 70

__C.CROSSNET.EXPANSION = 0.4

__C.CROSSNET.KERNEL_SIZE = 5

__C.CROSSNET.LAST1X1_GROUP = 2

__C.CROSSNET.POD_DEPTH = 3

__C.CROSSNET.BLOCK_COUNTS = (1, 1, 2, 2)

# Init residual transformation function
__C.CROSSNET.INIT_TRANS_FUNC = 'linear_bottleneck_transformation'

# Cross residual transformation function
__C.CROSSNET.CROSS_TRANS_FUNC = 'cross_bottleneck_transformation'

# CROSSNET's stem function (conv1, conv2, conv3)
__C.CROSSNET.STEM_FUNC = 'basic_bn_stem'

# Apply dilation in stage "crossnet5"
__C.CROSSNET.C5_DILATION = 1

# Freeze model weights before and including which block.
# Choices: [0, 2, 3, 4, 5]. O means not fixed. First conv and bn are defaults to
# be fixed.
__C.CROSSNET.FREEZE_AT = 2

# Use GroupNorm instead of BatchNorm
__C.CROSSNET.USE_GN = False

# ---------------------------------------------------------------------------- #
# MOBILENETV1 options
# ---------------------------------------------------------------------------- #
__C.MOBILENETV1 = AttrDict()

__C.MOBILENETV1.WIDEN_FACTOR = 1.0

__C.MOBILENETV1.KERNEL_SIZE = 3

__C.MOBILENETV1.ACTIVATION = 'relu'

__C.MOBILENETV1.BLOCK_COUNTS = (2, 2, 6, 2)

# Transformation function
__C.MOBILENETV1.TRANS_FUNC = 'basicblock_transformation'

# MOBILENETV1's stem function (conv1, conv2, conv3)
__C.MOBILENETV1.STEM_FUNC = 'basic_bn_stem'

# Apply dilation in stage "mobv1_5"
__C.MOBILENETV1.C5_DILATION = 1

# Freeze model weights before and including which block.
# Choices: [0, 2, 3, 4, 5]. O means not fixed. First conv and bn are defaults to
# be fixed.
__C.MOBILENETV1.FREEZE_AT = 2

# Use GroupNorm instead of BatchNorm
__C.MOBILENETV1.USE_GN = False

# ---------------------------------------------------------------------------- #
# MOBILENETV2 options
# ---------------------------------------------------------------------------- #
__C.MOBILENETV2 = AttrDict()

__C.MOBILENETV2.WIDEN_FACTOR = 1.0

__C.MOBILENETV2.INVERTED_T = 6

__C.MOBILENETV2.KERNEL_SIZE = 3

__C.MOBILENETV2.GROUPS = (1, 1)

__C.MOBILENETV2.ACTIVATION = 'relu6'

__C.MOBILENETV2.BLOCK_COUNTS = (2, 3, (4, 3), (3, 1))

# Residual transformation function
__C.MOBILENETV2.TRANS_FUNC = 'linear_bottleneck_transformation'

# MOBILENETV2's stem function (conv1, conv2, conv3)
__C.MOBILENETV2.STEM_FUNC = 'basic_bn_stem'

# Apply dilation in stage "mobv2_5"
__C.MOBILENETV2.C5_DILATION = 1

# Freeze model weights before and including which block.
# Choices: [0, 2, 3, 4, 5]. O means not fixed. First conv and bn are defaults to
# be fixed.
__C.MOBILENETV2.FREEZE_AT = 2

# Use GroupNorm instead of BatchNorm
__C.MOBILENETV2.USE_GN = False

# ---------------------------------------------------------------------------- #
# MNASNET options
# ---------------------------------------------------------------------------- #
__C.MNASNET = AttrDict()

__C.MNASNET.WIDEN_FACTOR = 1.0

__C.MNASNET.INVERTED_TS = (3, 3, (6, 6), (6, 6))

__C.MNASNET.KERNEL_SIZES = (3, 5, (5, 3), (5, 3))

__C.MNASNET.GROUPS = (1, 1)

__C.MNASNET.ACTIVATION = 'relu'

__C.MNASNET.BLOCK_COUNTS = (3, 3, (3, 2), (4, 1))

# Residual transformation function
__C.MNASNET.TRANS_FUNC = 'linear_bottleneck_transformation'

# MNASNET's stem function (conv1, conv2, conv3)
__C.MNASNET.STEM_FUNC = 'basic_bn_stem'

# Apply dilation in stage "mnas_5"
__C.MNASNET.C5_DILATION = 1

# Freeze model weights before and including which block.
# Choices: [0, 2, 3, 4, 5]. O means not fixed. First conv and bn are defaults to
# be fixed.
__C.MNASNET.FREEZE_AT = 2

# Use GroupNorm instead of BatchNorm
__C.MNASNET.USE_GN = False
...
__C.VGGS.BLOCK_FUNC = 'vgg_block'

__C.VGGS.USE_C5_POOL = False

__C.VGGS.FREEZE_AT = 2

# ---------------------------------------------------------------------------- #
# INCEPTIONV3 options
# ---------------------------------------------------------------------------- #
__C.INCEPTIONV3 = AttrDict()

__C.INCEPTIONV3.C5_STRIDE = 2

__C.INCEPTIONV3.ALIGN = False

__C.INCEPTIONV3.FREEZE_AT = 2
...
__C.CKPT = 'ckpts/mscoco/e2e_mask_rcnn_R-50-FPN_1x'
