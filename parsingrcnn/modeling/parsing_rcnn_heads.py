...
class roi_parsing_head_aspp_convx(nn.Module):
    """Mask R-CNN parsing head. v1convX design: X * (conv)."""
...
class roi_parsing_head_aspp_convx_gn(nn.Module):
    """Mask R-CNN parsing head. v1convX design: X * (conv)."""
    ...
    def detectron_weight_mapping(self):
        ...
        for i in range(cfg.PRCNN.NUM_CONVS_AFTER_ASPP):
            ...
            detectron_weight_mapping['conv_after_aspp.%d.weight' % (3 * i + 1)] = \
                'aspp_conv%d_gn_s' % (i + 1 + cfg.PRCNN.NUM_CONVS_BEFORE_ASPP)
            detectron_weight_mapping['conv_after_aspp.%d.bias' % (3 * i + 1)] = \
                'aspp_conv%d_gn_b' % (i + 1 + cfg.PRCNN.NUM_CONVS_BEFORE_ASPP)
...
class roi_parsing_head_aspp_convx_adp(nn.Module):
    """Mask R-CNN parsing head. v1convX design: X * (conv) with adp."""
    ...
    def __init__(self, dim_in, roi_xform_func, spatial_scale):
        ...
        self.parsing_conv = nn.ModuleList()
        self.num_levels = cfg.FPN.ROI_MAX_LEVEL - cfg.FPN.ROI_MIN_LEVEL + 1
        for i in range(self.num_levels):
            self.parsing_conv.append(nn.Sequential(
                nn.Conv2d(dim_in, hidden_dim, kernel_size, 1, pad_size),
                nn.ReLU(inplace=True)
            ))
        dim_in = hidden_dim
    ...
    def detectron_weight_mapping(self):
        ...
        for i in range(self.num_levels):
            detectron_weight_mapping.update({
                'parsing_conv.{}.0.weight'.format(i): 'panet{}_parsing_w'.format(i+1),
                'parsing_conv.{}.0.bias'.format(i): 'panet{}_parsing_b'.format(i + 1)
            })
...
class roi_parsing_head_aspp_convx_nonlocal(nn.Module):
    """Mask R-CNN parsing head. v1convX design: X * (conv) with nonlocal."""
