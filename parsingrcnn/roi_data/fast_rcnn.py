...
import parsingrcnn.roi_data.keypoint_rcnn as roi_data_keypoint_rcnn
import parsingrcnn.roi_data.mask_rcnn as roi_data_mask_rcnn
import parsingrcnn.roi_data.parsing_rcnn as roi_data_parsing_rcnn
import parsingrcnn.roi_data.uv_rcnn as  roi_data_uv_rcnn
...
def get_fast_rcnn_blob_names(is_training=True):
    ...
    if is_training and cfg.MODEL.PARSING_ON:
        # 'parsing_rois': RoIs sampled for training the mask prediction branch.
        # Shape is (#parsings, 5) in format (batch_idx, x1, y1, x2, y2).
        blob_names += ['parsing_rois']
        # 'roi_has_parsing': binary labels for the RoIs specified in 'rois'
        # indicating if each RoI has a parsing or not. Note that in some cases
        # a *bg* RoI will have an all -1 (ignore) parsing associated with it in
        # the case that no fg RoIs can be sampled. Shape is (batchsize).
        blob_names += ['parsing_weights']
        # 'parsings_int32' holds binary parsings for the RoIs specified in
        # 'parsing_rois'. Shape is (#fg, M * M) where M is the ground truth
        # parsing size.
        blob_names += ['parsing_int32']
    if is_training and cfg.MODEL.UV_ON:
        blob_names += ['uv_rois']
        blob_names += ['roi_has_uv_int32']
        #########
        # ###################################################
        blob_names += ['uv_ann_labels']
        blob_names += ['uv_ann_weights']
        # #################################################
        blob_names += ['uv_X_points']
        blob_names += ['uv_Y_points']
        blob_names += ['uv_Ind_points']
        blob_names += ['uv_I_points']
        blob_names += ['uv_U_points']
        blob_names += ['uv_V_points']
        blob_names += ['uv_point_weights']
    if cfg.FPN.FPN_ON and cfg.FPN.MULTILEVEL_ROIS:
        ...
        if is_training:
            if cfg.MODEL.MASK_ON \
                    and not cfg.MRCNN.FINEST_LEVEL_ROI:
                for lvl in range(k_min, k_max + 1):
                    blob_names += ['mask_rois_fpn' + str(lvl)]
                blob_names += ['mask_rois_idx_restore_int32']
            if cfg.MODEL.KEYPOINTS_ON \
                    and not cfg.KRCNN.FINEST_LEVEL_ROI:
                for lvl in range(k_min, k_max + 1):
                    blob_names += ['keypoint_rois_fpn' + str(lvl)]
                blob_names += ['keypoint_rois_idx_restore_int32']
            if cfg.MODEL.PARSING_ON \
                    and not cfg.PRCNN.FINEST_LEVEL_ROI:
                for lvl in range(k_min, k_max + 1):
                    blob_names += ['parsing_rois_fpn' + str(lvl)]
                blob_names += ['parsing_rois_idx_restore_int32']
            if cfg.MODEL.UV_ON \
                    and not cfg.UVRCNN.FINEST_LEVEL_ROI:
                for lvl in range(k_min, k_max + 1):
                    blob_names += ['uv_rois_fpn' + str(lvl)]
                blob_names += ['uv_rois_idx_restore_int32']
    return blob_names
...
def add_fast_rcnn_blobs(blobs, im_scales, roidb):
    ...
    valid = True
    if cfg.MODEL.KEYPOINTS_ON:
        valid = roi_data_keypoint_rcnn.finalize_keypoint_minibatch(blobs, valid)
...
def _sample_rois(roidb, im_scale, batch_idx):
    ...
    if cfg.MODEL.MASK_ON:
        roi_data_mask_rcnn.add_mask_rcnn_blobs(blob_dict, sampled_boxes, roidb,
                                               im_scale, batch_idx)

    # Optionally add Keypoint R-CNN blobs
    if cfg.MODEL.KEYPOINTS_ON:
        roi_data_keypoint_rcnn.add_keypoint_rcnn_blobs(
            blob_dict, roidb, fg_rois_per_image, fg_inds, im_scale, batch_idx)

    if cfg.MODEL.PARSING_ON:
        roi_data_parsing_rcnn.add_parsing_rcnn_blobs(
            blob_dict, sampled_boxes, roidb, im_scale, batch_idx
        )

    if cfg.MODEL.UV_ON:
        roi_data_uv_rcnn.add_uv_rcnn_blobs(
            blob_dict, sampled_boxes, roidb, im_scale, batch_idx
        )
...
def _add_multilevel_rois(blobs):
    ...
    if cfg.MODEL.PARSING_ON \
            and not cfg.PRCNN.FINEST_LEVEL_ROI:
        _distribute_rois_over_fpn_levels('parsing_rois')
    if cfg.MODEL.UV_ON \
            and not cfg.UVRCNN.FINEST_LEVEL_ROI:
        _distribute_rois_over_fpn_levels('uv_rois')
