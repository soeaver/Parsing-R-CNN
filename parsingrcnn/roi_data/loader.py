...
from parsingrcnn.roi_data.minibatch import get_minibatch
...
def collate_minibatch(list_of_blobs):
    ...
    for i in range(0, len(list_of_blobs), cfg.TRAIN.IMS_PER_BATCH):
        ...
        if cfg.RETINANET.RETINANET_ON:
            mini_list = pad_key_in_blobs(mini_list)
...
def pad_key_in_blobs(list_of_blobs):
    for k in list_of_blobs[0]:
        if not (k.find('retnet_roi_bbox_targets') >= 0 or k.find('retnet_roi_fg_bbox_locs') >= 0):  
            # becasue (4,) maybe returned if only one found
            continue
        for blob in list_of_blobs:  # deal with (4, ) from unqueeze
            if blob[k].shape == (4,):
                blob[k] = np.expand_dims(blob[k], axis=0)
        max_shape = np.array([blobs[k].shape for blobs in list_of_blobs if blobs[k].shape != (4,)]).max(axis=0)

        if max_shape[0] == 0:  # deal with empty (0,4)
            max_shape = (1, 4)
        for blob in list_of_blobs:
            data_padded = np.zeros((max_shape[0], max_shape[1]), dtype=np.float32)
            h, w = blob[k].shape
            data_padded[:h, :w] = blob[k]
            blob[k] = data_padded
