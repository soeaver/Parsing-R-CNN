...
import parsingrcnn.utils.segms as segm_utils
...
class JsonDataset(object):
    ...
    def _prep_roidb_entry(self, entry):
        ...
        if cfg.MODEL.UV_ON:
            im_path = im_path.replace(self.image_prefix, '')
        assert os.path.exists(im_path), 'Image \'{}\' not found'.format(im_path)
        entry['image'] = im_path
        entry['flipped'] = False
        entry['has_visible_keypoints'] = False
        entry['has_uv'] = False
        entry['has_parsing'] = False
        # Empty placeholders
        ...
        entry['difficult'] = np.empty((0), dtype=np.int32)
    ...
    def _add_gt_annotations(self, entry):
        ...
        for obj in objs:
            ...
            if obj['area'] > 0 and x2 > x1 and y2 > y1:
                ...
                ###
                if 'parsing' in obj.keys():
                    parsing_directory = dataset_catalog.get_parsing_dir(self.name)
                    parsing_path = os.path.join(
                        parsing_directory, obj['parsing'])
                    assert os.path.exists(parsing_path), \
                        'Image \'{}\' not found'.format(parsing_path)
                    valid_parsing.append(parsing_path)
                else:
                    valid_parsing.append('0')
        num_valid_objs = len(valid_objs)

        boxes = np.zeros((num_valid_objs, 4), dtype=entry['boxes'].dtype)
        gt_classes = np.zeros((num_valid_objs), dtype=entry['gt_classes'].dtype)
        difficult = np.zeros((num_valid_objs), dtype=entry['difficult'].dtype)
        ...
        for ix, obj in enumerate(valid_objs):
            ...
            gt_classes[ix] = cls
            if 'difficult' in obj:
                difficult[ix] = int(obj['difficult'])
            else:
                difficult[ix] = 0
        ...
        entry['parsing'].extend(valid_parsing)
        ...
        entry['difficult'] = np.append(entry['difficult'], difficult)
