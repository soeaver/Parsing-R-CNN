...
class roi_pose_head_aspp_convx(nn.Module):
    """Mask R-CNN keypoint head. v1convX design: X * (conv)."""
    def __init__(self, dim_in, roi_xform_func, spatial_scale):
        super().__init__()
        self.dim_in = dim_in
...
class roi_pose_head_aspp_convx_gn(nn.Module):
    """Mask R-CNN keypoint head. v1convX design: X * (conv)."""
    def __init__(self, dim_in, roi_xform_func, spatial_scale):
        super().__init__()
        self.dim_in = dim_in
    ...
    def detectron_weight_mapping(self):
        ...
        for i in range(cfg.KRCNN.NUM_CONVS_AFTER_ASPP):
            ...
            detectron_weight_mapping['conv_after_aspp.%d.weight' % (3*i+1)] = \
                'aspp_conv%d_gn_s' % (i + 1 + cfg.KRCNN.NUM_CONVS_BEFORE_ASPP)
            detectron_weight_mapping['conv_after_aspp.%d.bias' % (3*i+1)] = \
                'aspp_conv%d_gn_b' % (i + 1 + cfg.KRCNN.NUM_CONVS_BEFORE_ASPP)
...
class roi_pose_head_aspp_convx_nonlocal(nn.Module):
    """Mask R-CNN pose head. v1convX design: X * (conv) with nonlocal."""
