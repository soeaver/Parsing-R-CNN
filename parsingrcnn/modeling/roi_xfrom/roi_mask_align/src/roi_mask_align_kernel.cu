...
#include <math.h>
#include <float.h>
#include "roi_mask_align_kernel.h"
...
#define CUDA_1D_KERNEL_LOOP(i, n)                              \
    ...
    __global__ void ROIMaskAlignForward(const int nthreads, 
        ...
        CUDA_1D_KERNEL_LOOP(index, nthreads) {
            ...
            float w = offset_bottom_rois[3] - offset_bottom_rois[1] + 1.;
            float h = offset_bottom_rois[4] - offset_bottom_rois[2] + 1.;
            float xc = offset_bottom_rois[1] + 0.5 * w;
            float yc = offset_bottom_rois[2] + 0.5 * h;

            // rescale roi with regard to roi_scale and half_part
            float xx1 = xc - 0.5 * w * roi_scale;
            float xx2 = xc + 0.5 * w * roi_scale - 1.;
            float yy1 = yc - 0.5 * h * roi_scale;
            float yy2 = yc + 0.5 * h * roi_scale - 1.;
            switch(half_part) {
                case 0: break;
                case 1: xx2 = xc; break;
                case 2: xx1 = xc; break;
            ...
            float roi_start_w = xx1 * spatial_scale + spatial_shift;
            float roi_start_h = yy1 * spatial_scale + spatial_shift;
            float roi_end_w = xx2 * spatial_scale + spatial_shift;
            float roi_end_h = yy2 * spatial_scale + spatial_shift;
            ...
            for (int iy = 0; iy < roi_bin_grid_h; iy++) // e.g., iy = 0, 1
            {
                ...
                for (int ix = 0; ix < roi_bin_grid_w; ix++) {
                    ...
                    float val = bilinear_interpolate(offset_bottom_data,
                                                     height, 
                                                     width, 
                                                     y, 
                                                     x, 
    ...
    int ROIMaskAlignForwardLaucher(const float* bottom_data, 
        ...
        ROIMaskAlignForward
            <<<(output_size + kThreadsPerBlock - 1) / kThreadsPerBlock, kThreadsPerBlock, 0, stream>>>(
                output_size,
                bottom_data, 
                spatial_scale, 
                height, 
