...
from rcnn.modeling.parsing_rcnn.parsingiou.parsingiou import ParsingIoU
...
class ParsingRCNN(torch.nn.Module):
    ...
    def _forward_train(self, conv_features, proposals, targets=None):
        ...
        if cfg.PRCNN.PARSINGIOU_ON:
            loss_parsing, parsingiou_targets = self.loss_evaluator(parsing_logits)
            loss_parsingiou, all_proposals = self.ParsingIoU(
                x, all_proposals, parsing_logits, parsingiou_targets
            )
            return x, all_proposals, dict(loss_parsing=loss_parsing, loss_parsingiou=loss_parsingiou)
        else:
