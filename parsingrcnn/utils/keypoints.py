...
def fast_heatmaps_to_keypoints(maps, rois):
    """Extract predicted keypoint locations from heatmaps. Output has shape
    (#rois, 4, #keypoints) with the 4 rows corresponding to (x, y, logit, prob)
    for each keypoint.
    """
    ...
    # NCHW to NHWC for use with OpenCV
    maps = np.transpose(maps, [0, 2, 3, 1])
    min_size = cfg.KRCNN.INFERENCE_MIN_SIZE  # default 0
    xy_preds = np.zeros(
        (len(rois), 4, cfg.KRCNN.NUM_KEYPOINTS), dtype=np.float32)
    for i in range(len(rois)):
        if min_size > 0:
        ...
        #     interpolation=cv2.INTER_CUBIC)
        # Bring back to CHW
        roi_map = maps[i].copy()  # add
        roi_map = np.transpose(roi_map, [2, 0, 1])  # HWC to CHW
        roi_map_probs = scores_to_probs(roi_map.copy())
        w = roi_map.shape[2]
        for k in range(cfg.KRCNN.NUM_KEYPOINTS):
            pos = roi_map[k, :, :].argmax()
            ...
            assert (roi_map_probs[k, y_int, x_int] ==
                    roi_map_probs[k, :, :].max())
            x = (x_int * float(roi_map_width / w) + 0.5) * width_correction  # add
            y = (y_int * float(roi_map_height / w) + 0.5) * height_correction  # add
            # x = (x_int + 0.5) * width_correction
            # y = (y_int + 0.5) * height_correction
            xy_preds[i, 0, k] = x + offset_x[i]
            xy_preds[i, 1, k] = y + offset_y[i]
...
def gauss_heatmaps_to_keypoints(maps, rois):
    """Extract predicted keypoint locations from heatmaps. Output has shape
    (#rois, 4, #keypoints) with the 4 rows corresponding to (x, y, logit, prob)
    for each keypoint.
    """
    ...
    for i in range(len(rois)):
        ...
        for k in range(cfg.KRCNN.NUM_KEYPOINTS):
            border = 10
            single_map = roi_map[k] / np.amax(roi_map[k])
            dr = np.zeros((int(roi_map_height) + 2*border, 
                int(roi_map_width) + 2*border))

            dr[border:-border, border:-border] = single_map.copy()
            ...
            if ln > 1e-3:
                x += delta * px / ln
                y += delta * py / ln
            x = max(0, min(x, roi_map_width))
            y = max(0, min(y, roi_map_height))
            xy_preds[i, 0, k] = x + offset_x[i]
            xy_preds[i, 1, k] = y + offset_y[i]
            xy_preds[i, 2, k] = roi_map[k, y_int, x_int]
            xy_preds[i, 3, k] = roi_map_probs[k, y_int, x_int]
...
def keypoints_to_gauss_heatmap_labels(keypoints, rois):
    """Encode keypoint location in the target heatmap for use in
    SoftmaxWithLoss.
    """
    # Maps keypoints from the half-open interval [x1, x2) on continuous image
    ...
    shape = (len(rois), cfg.KRCNN.NUM_KEYPOINTS)
    heatmaps_shape = (
        len(rois), cfg.KRCNN.NUM_KEYPOINTS,
        cfg.KRCNN.HEATMAP_SIZE, cfg.KRCNN.HEATMAP_SIZE
    )
    heatmaps = blob_utils.zeros(heatmaps_shape)
    weights = blob_utils.zeros(shape)
    ...
    for kp in range(keypoints.shape[2]):
        ...
        sigma = (7, 7)
        for _i in range(len(valid)):
            if valid[_i] > 0:
                heatmaps[_i, kp, int(y[_i]), int(x[_i])] = 1
                heatmaps[_i, kp] = cv2.GaussianBlur(
                    heatmaps[_i, kp], sigma, 0
                )
                am = np.amax(heatmaps[_i, kp])
                heatmaps[_i, kp] /= am / 255
