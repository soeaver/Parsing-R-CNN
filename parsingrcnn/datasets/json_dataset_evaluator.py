...
from parsingrcnn.datasets.densepose_cocoeval import denseposeCOCOeval
...
def evaluate_uv(
    ...
    res_file = os.path.join(
        output_dir, 'uv_' + json_dataset.name + '_results'
    )
    if use_salt:
        res_file += '_{}'.format(str(uuid.uuid4()))
    res_file += '.pkl'
    results = _write_coco_uv_results_file(
        json_dataset, all_boxes, all_uvs, res_file
    )
    # Only do evaluation on non-test sets (annotations are undisclosed on test)
    if json_dataset.name.find('test') == -1:
        # See comment in _write_coco_uv_results_file
        #coco_eval = _do_uv_eval(json_dataset, res_file, output_dir)
        coco_eval = _do_uv_eval(json_dataset, results, output_dir)
    else:
        coco_eval = None
    # See comment in _write_coco_uv_results_file
    # Optionally cleanup results json file
...
def _write_coco_uv_results_file(
    json_dataset, all_boxes, all_uvs, res_file
):
    results = []
    for cls_ind,cls in enumerate(json_dataset.classes):
        if cls == '__background__':
            continue
        if cls_ind >= len(all_uvs):
            break
        logger.info(
            'Collecting {} results ({:d}/{:d})'.format(
                cls, cls_ind, len(all_uvs) - 1))
        cat_id = json_dataset.category_to_id_map[cls]
        results.extend(_coco_uv_results_one_category(
            json_dataset, all_boxes[cls_ind], all_uvs[cls_ind], cat_id))
    # UV results are stored in 3xHxW ndarray format,
    # which is not json serializable
    #logger.info(
    #    'Writing uv results json to: {}'.format(
    #        os.path.abspath(res_file)))
    #with open(res_file, 'w') as fid:
    #    json.dump(results, fid)
    #
    logger.info(
        'Writing uv results pkl to: {}'.format(
            os.path.abspath(res_file)))

    with open(res_file, 'wb') as f:
        pickle.dump(results, f, pickle.HIGHEST_PROTOCOL)
    #logger.info('Not writing uv resuts json')
    return res_file


def _coco_uv_results_one_category(json_dataset, boxes, uvs, cat_id):
    results = []
    image_ids = json_dataset.COCO.getImgIds()
    image_ids.sort()
    assert len(uvs) == len(image_ids)
    assert len(boxes) == len(image_ids)
    #
    for i, image_id in enumerate(image_ids):
        if len(boxes[i]) == 0 or len(uvs[i]) == 0:
            continue
        uv_dets = uvs[i]
...
def _do_uv_eval(json_dataset, res_file, output_dir):
    ann_type = 'uv'
    imgIds = json_dataset.COCO.getImgIds()
    imgIds.sort()
    ...
    coco_eval.evaluate()
    coco_eval.accumulate()
    #eval_file = os.path.join(output_dir, 'uv_results.pkl')
    #save_object(coco_eval, eval_file)
    #logger.info('Wrote json eval results to: {}'.format(eval_file))
    coco_eval.summarize()
    return coco_eval
