...
import parsingrcnn.roi_data.data_utils as data_utils
...
def add_retinanet_blobs(blobs, im_scales, roidb, image_width, image_height):  # this is called each iteration
    """Add RetinaNet blobs."""
...
def _get_retinanet_blobs(
    ...
    if len(gt_boxes) > 0:
        ...
        anchor_by_gt_overlap = box_utils.bbox_overlaps(anchors, gt_boxes)  # (371349, 17)
        # Map from anchor to gt box that has highest overlap
        anchor_to_gt_argmax = anchor_by_gt_overlap.argmax(axis=1)  # (371349,) this is index
        # For each anchor, amount of overlap with most overlapping gt box
        anchor_to_gt_max = anchor_by_gt_overlap[  # (371349,) this is area
            np.arange(num_inside), anchor_to_gt_argmax]

        # Map from gt box to an anchor that has highest overlap
        gt_to_anchor_argmax = anchor_by_gt_overlap.argmax(axis=0)  # (17,)  index
        ...
        labels[inds] = gt_classes[gt_inds]  # for all anchors, inds are valued by gt_inds, this gives class values 1~80
    ...
    for foa in foas:
        ...
        if _roi_bbox_targets.astype(np.float32).shape[0] == 0 and _roi_fg_bbox_locs.astype(np.float32).shape[0] == 0:
            blobs_out.append(
                dict(
