...
def get_dataset(ds_name):
    assert os.path.exists(dataset_catalog.get_ann_fn(ds_name)), \
        'Ann fn \'{}\' not found'.format(dataset_catalog.get_ann_fn(ds_name))

    COCOAPI = COCO(dataset_catalog.get_ann_fn(ds_name))

    category_ids = COCOAPI.getCatIds()
    categories = [c['name'] for c in COCOAPI.loadCats(category_ids)]
